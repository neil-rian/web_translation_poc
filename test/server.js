...
// Save updated English source
app.post('/api/save-source', (req, res) => {
    const { texts } = req.body;
    if (!texts) return res.status(400).json({ error: 'Missing texts' });

    writeLangFile('en', texts);
    console.log(`Saved en.json (${Object.keys(texts).length} keys)`);
    res.json({ success: true, keys: Object.keys(texts).length });
});
